...
import static com.st.st25nfc.generic.ndef.NDEFEditorFragment.DELETE_NDEF_RECORD;
...
public class NDEFActivity extends STFragmentActivity implements NavigationView.OnNavigationItemSelectedListener,
                                                                ...
                                                                STType2PwdDialogFragment.STType2PwdDialogListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mNDEFRecord instanceof TextRecord) {
            ...
            if ((myTag instanceof ST25TVCTag) && (mArea == 1) && (mNdefMsg.getNbrOfRecords() == 1)) {
                mFragment = ST25TVCUriFragment.newInstance(context);
            } else if ((myTag instanceof ST25TNTag) && (mArea == 1) && (mNdefMsg.getNbrOfRecords() == 1)) {
                mFragment = ST25TNANDEFUriFragment.newInstance(context);
            } else {
    ...
    public void hookWriteActions(WriteNdefMessageHook hook) {
        writeNdefMessageHook = hook;
    }
            ...
            case R.id.action_delete:
                if(mAction == NDEFEditorFragment.ADD_NDEF_RECORD) {
                    ...
                    alertDialogBuilder
                            ...
                            .setPositiveButton(getString(R.string.yes), new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog, int id) {
                                    mAction = DELETE_NDEF_RECORD;
                                    // Should not arrive but nothing to do if no Ndef
                                    if (mNdefMsg != null) {
                                        mNdefMsg.deleteRecord(mRecordNbr);
                                        writeNdefMessage();
                                    } else {
                                        Log.e(TAG, "Ndef message is null!");
                                    }
                                }
                            })
                            .setNegativeButton(getString(R.string.no),new DialogInterface.OnClickListener() {
    ...
    private void writeNdefMessage() {
        ...
        if (writeNdefMessageHook != null) {
            // Code for tags with Augmented NDEF needing a special procedure when Writing a NDEF
            if (mAction == DELETE_NDEF_RECORD) {
                // "res" is true if the event was consumed and false otherwise
                boolean res = writeNdefMessageHook.deleteNdefMessage();
                if (res) {
                    return;
                }
            } else {
                // "res" is true if the event was consumed and false otherwise
                boolean res = writeNdefMessageHook.writeNdefMessage();
                if (res) {
                    return;
                }
            }
    ...
    public void showWritePasswordDialog() {
        ...
        } else if(UIHelper.isAType2Tag(myTag)) {
            new AsyncTaskDisplayPasswordDialogBoxForType2Tag().execute();
    ...
    private void showNdefTooBigAlert() {
        ...
        if(memoryAreaSizeInBytes != 0) {
            if (myTag instanceof Type4Tag) {
                ...
                Type5Tag type5Tag = (Type5Tag) myTag;
                int ccfileLength = 4;
                try {
                    ccfileLength = type5Tag.getExpectedCCFileLength();
                } catch (STException e) {
                    e.printStackTrace();
                }
                ...
                message += "\n";
            } else if (myTag instanceof Type2Tag) {
                message += "\n\n";
                message +=  getString(R.string.ndef_size_in_bytes,ndefSizeInBytes);
                message += "\n";
                message += getString(R.string.max_file_size_in_bytes,memoryAreaSizeInBytes);
    ...
    public void onSTType2PwdDialogFinish(int result) {
        Log.v(TAG, "onSTType2PwdDialogFinish. result = " + result);
    ...
    private class AsyncTaskWriteNdefMessage extends AsyncTask<Void, Void, ActionStatus> {
        ...
        protected ActionStatus doInBackground(Void... param) {
            ...
            try {
                UIHelper.displayCircularProgressBar(NDEFActivity.this, getString(R.string.please_wait));
    ...
    /**
     * AsyncTask retrieving the passwordNumber corresponding to an area of a Type2 tag.
     * When the password number is available, a password dialog box is displayed.
     */
    private class AsyncTaskDisplayPasswordDialogBoxForType2Tag extends AsyncTask<Void, Void, ActionStatus> {
        ...
        protected ActionStatus doInBackground(Void... param) {
            ...
            if (myTag instanceof STType2PasswordInterface) {
                mPasswordNumber = UIHelper.getConfigurationPasswordNumber(myTag);
                result = ActionStatus.ACTION_SUCCESSFUL;
            } else {
                if (myTag instanceof STType2PasswordInterface) {
                    // TODO:
        ...
        protected void onPostExecute(ActionStatus actionStatus) {
            ...
            switch(actionStatus) {
                case ACTION_SUCCESSFUL:
                    ...
                    STType2PwdDialogFragment pwdDialogFragment = STType2PwdDialogFragment.newInstance(
                            STType2PwdDialogFragment.STPwdAction.PRESENT_CURRENT_PWD,
                            mPasswordNumber,
                            dialogMsg,
                            NDEFActivity.this);
                    if(pwdDialogFragment!=null) pwdDialogFragment.show(fragmentManager, "pwdDialogFragment");
