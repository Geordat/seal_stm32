...
public class CounterConfigActivity extends STFragmentActivity implements NavigationView.OnNavigationItemSelectedListener, CompoundButton.OnCheckedChangeListener {
    ...
    private void enableCounterFeatures(boolean enable) {
        ...
        counterStates.add(getString(R.string.increment_on_read));
        counterStates.add(getString(R.string.increment_on_write));
                ...
                try {
                    switch (selection) {
                        ...
                        case 1:
                            ...
                            break;
                        default:
                            showToast(R.string.wrong_feature);
                            break;
                        // none selected
                    }
            ...
            public void run() {
                ...
                try {
                    if (((STType4CounterInterface) myTag).isCounterLocked()) {
                        showToast(R.string.tag_already_locked);
                    } else {
                        // The lock cannot be removed .... WARNING
                        //((STType4CounterInterface) myTag).lockCounter();
                        showToast(R.string.tag_updated );
                        showToast(R.string.warning_cmd_not_executed_reverse_not_available);
                    }
                } catch (STException e) {
                    switch (e.getError()) {
                        case TAG_NOT_IN_THE_FIELD:
    ...
    private void unlockCounter() {
        new Thread(new Runnable() {
            public void run() {
                ...
                try {
                    if (((STType4CounterInterface) myTag).isCounterLocked()) {
                        showToast(R.string.counter_locked_unlock_impossible);
                    } else {
    ...
    private void displayLockConfirmationDialogBox() {
        ...
        new AlertDialog.Builder(this)
                ...
                .setMessage(getString(R.string.lock_configuration_confirmation))
