...
public class ST25DVFtmDemoActivity extends STFragmentActivity implements NavigationView.OnNavigationItemSelectedListener, FtmProtocol.TransferProgressionListener {
    ...
    private byte[] b = new byte[65536*6];
    byte[] c ;
    private int cnt=0;
    private  String bt = "";
    ...
    private void pleaseSelectDataSizeOnDiscovery() {
        ...
        // set dialog message
        alertDialogBuilder.setMessage(getString(R.string.please_select_a_data_size_on_discovery)).setCancelable(true)
        ...
        protected ActionStatus doInBackground(Void... param) {
            ...
            try {
                switch (mAction) {
                    ...
                    case READ_DATA:
                        startTimer();
                        cmdId = mIsErrorRecoveryEnabled ? FTM_CMD_READ_DATA : FTM_CMD_READ_DATA_NO_ERROR_RECOVERY;
                        ...
                        for(int i=32768*cnt;i<(32768*(cnt+1));i++){
                            b[i] = mDataReceived[i-32768*cnt];
                        }
                        if(cnt==11){
                    ...
                    case SEND_DATA:
                        startTimer();
                        cmdId = mIsErrorRecoveryEnabled ? FTM_CMD_READ_DATA : FTM_CMD_READ_DATA_NO_ERROR_RECOVERY;
                        mDataReceived = mFtmCommands.sendCmdAndWaitForCompletion(FTM_CMD_FW_UPGRADE, null, mIsErrorRecoveryEnabled, true, ST25DVFtmDemoActivity.this, FtmCommands.DEFAULT_FTM_TIME_OUT_IN_MS);

                        b = Arrays.copyOf(mDataReceived, mDataReceived.length);
