...
void re_seal_save(string fpath,  RelinKeys &relin_keys, bool compress)
{
    ...
    cerr << action << " ..." << endl;
    if (compress)
        relin_keys.save(file, compr_mode_type::zstd);
    else
        relin_keys.save(file, compr_mode_type::none);
    file.close();
}
...
void pm_seal_save(string fpath,  EncryptionParameters &parms, bool compress)
{
    ...
    cerr << action << " ..." << endl;
    if (compress)
        parms.save(file, compr_mode_type::zstd);
    else
        parms.save(file, compr_mode_type::none);
    file.close();
}
...
void gal_seal_save(string fpath, GaloisKeys &gal_keys, bool compress)
{
    string action = "";
    ...
    cerr << action << " ..." << endl;
    if (compress)
        gal_keys.save(file, compr_mode_type::zstd);
    else
        gal_keys.save(file, compr_mode_type::none);
    file.close();
}
